...
void print_exp(Expression *exp)
{
    ...
    switch (exp->type)
    {
        ...
        case ExpType_CALL:
            ...
            break;
        case ExpType_UNARY:
            fprintf(stdout, "%s", UnaryOp_get_str(exp->unary.op));
            print_exp(exp->unary.operand);
            break;
        case ExpType_BINARY:
            fprintf(stdout, "(");
            print_exp(exp->binary.left);
            fprintf(stdout, " %s ", BinaryOp_get_str(exp->binary.op));
...
void print_node(Node *node)
{
    ...
    switch (node->type)
    {
        ...
        case NodeType_ASSIGN:
            ...
            break;
        case NodeType_DECL:
            fprintf(stdout, "%s %s", Type::get_str(node->decl.var_type),
        ...
        case NodeType_FUNC_DEF:
        {
            ...
            if (node->func_def.params)
            {
                ...
                while (true)
                {
                    fprintf(stdout, "%s %s", Type::get_str(param->type), param->name.data);
                    param = param->next;
                    if (!param) break;
                    fprintf(stdout, ", ");
                }
